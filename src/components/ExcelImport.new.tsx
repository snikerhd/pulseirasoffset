...
function normalizeExcelValue(value: unknown): string {
  ...
  if (typeof value === 'object') {
    const maybeText = (value as { w?: string }).w;
...
function extractPulseiraCode(value: unknown): string {
  const str = normalizeExcelValue(value).toUpperCase();
  if (!str) return '';

  const match = str.match(/([A-Z]{2,4}\d{3,5})/);
...
function detectColumns(rows: Record<string, unknown>[]): { pulseiraCol: string | null; nomeCol: string | null } {
  ...
  const pulseiraKeywords = ['pulseira', 'pulseira', 'id', 'codigo', 'código', 'code', 'tag', 'rfid'];
  const nomeKeywords = ['nome', 'name', 'acusado', 'pessoa', 'person', 'suspeito', 'suspect', 'player', 'jogador'];
  ...
  for (let i = 0; i < cols.length; i++) {
    ...
    if (!pulseiraCol && pulseiraKeywords.some((k) => lower.includes(k))) pulseiraCol = cols[i];
    if (!nomeCol && nomeKeywords.some((k) => lower.includes(k))) nomeCol = cols[i];
  }
  ...
  if (!pulseiraCol || !nomeCol) {
    ...
    for (const col of cols) {
      ...
      const values = sample.map((r) => normalizeExcelValue(r[col])).filter(Boolean);
      if (!values.length) continue;

      const pulseiraRegex = /^[A-Z]{2,4}\d{3,5}$/;
      const pulseiraCount = values.filter((v) => pulseiraRegex.test(v)).length;
      const nomeCount = values.filter((v) => v.length > 2 && !pulseiraRegex.test(v) && /[a-zA-ZÀ-ÿ]/.test(v)).length;

      if (!pulseiraCol && pulseiraCount >= Math.ceil(sample.length / 2)) pulseiraCol = col;
      if (!nomeCol && nomeCount >= Math.ceil(sample.length / 2)) nomeCol = col;
    }
  }
...
export default function ExcelImport({ pulseiras, setPulseiras, addLog, onClose }: ExcelImportProps) {
  ...
  const [preview, setPreview] = useState<{ row: number; pulseira: string; nome: string }[]>([]);
  ...
  const updatePreview = useCallback(
    (data: Record<string, unknown>[], pCol: string, nCol: string) => {
      const previewRows = data.slice(0, 10).map((r, i) => ({
        row: i + 2,
        pulseira: pCol ? extractPulseiraCode(r[pCol]) : '',
        nome: nCol ? normalizeExcelValue(r[nCol]) : '',
      }));
  ...
  const handleImport = () => {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const nome = nomeCol ? normalizeExcelValue(r[nomeCol]) || `Linha ${i + 2}` : `Linha ${i + 2}`;
